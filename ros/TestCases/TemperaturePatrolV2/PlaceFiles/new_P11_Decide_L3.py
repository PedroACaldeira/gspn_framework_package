...
class MinimalActionServer(object):
    ...
    def __init__(self, name):
        ...
        self._as.start()
        rospy.loginfo('DECIDE L3 SERVER : MEGA ONLINE')
        self._as.start()
    ...
    def goal_callback(self, goal_request):
        ...
        self.get_logger().info('DECIDE L3 SERVER : Received goal request')
        return GoalResponse.ACCEPT

    def cancel_callback(self, goal_handle):
        """Accepts or rejects a client request to cancel an action."""
        self.get_logger().info('DECIDE L3 SERVER : Received cancel request')
        return CancelResponse.ACCEPT

    def execute_callback(self, goal):
        print("P11 immediate place running.")
        self._result.transition = 'None'
        rospy.loginfo('DECIDE L3 SERVER : Returning result: {0}'.format(self._result.transition))
