...
class MinimalActionServer(object):
    ...
    def __init__(self, name, _namespace):
        ...
        self._as.start()
        rospy.loginfo('WAIT L2 SERVER : ONLINE')
        self._as.start()
    ...
    def goal_callback(self, goal_request):
        ...
        self.get_logger().info('WAIT L2 SERVER : Received goal request')
        return GoalResponse.ACCEPT

    def cancel_callback(self, goal_handle):
        """Accepts or rejects a client request to cancel an action."""
        self.get_logger().info('WAIT L2 SERVER : Received cancel request')
        return CancelResponse.ACCEPT

    def execute_callback(self, goal):
        """Executes a goal."""
        rospy.loginfo('WAIT L2 SERVER : WAITING ON L2...')
        ...
        for i in range(1, 6):
            if self._as.is_preempt_requested():
                ...
                success = False
                rospy.loginfo('WAIT L2 SERVER : Goal canceled')
                break
        ...
        if success:
            self._result.transition = 't11:Wait_Done_L2'
