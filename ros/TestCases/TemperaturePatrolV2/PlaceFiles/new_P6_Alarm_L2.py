...
class MinimalActionServer(object):
    ...
    def __init__(self, name):
        ...
        self._as.start()
        rospy.loginfo('ALARM L2 SERVER : ONLINE')
        self._as.start()
    ...
    def goal_callback(self, goal_request):
        ...
        self.get_logger().info('ALARM L2 SERVER : Received goal request')
        return GoalResponse.ACCEPT

    def cancel_callback(self, goal_handle):
        """Accepts or rejects a client request to cancel an action."""
        self.get_logger().info('ALARM L2 SERVER : Received cancel request')
        return CancelResponse.ACCEPT

    def execute_callback(self, goal):
        """Executes a goal."""
        rospy.loginfo('ALARM L2 SERVER : STARTING ALARM L2...')
        ...
        for i in range(1, 8):
            if self._as.is_preempt_requested():
                ...
                success = False
                rospy.loginfo('ALARM L2 SERVER : Goal canceled')
                break
            # Update Fibonacci sequence
            self._feedback.time_passed.append(i)

            rospy.loginfo('ALARM L2 SERVER : TEMPERATURE TOO HIGH IN L2: {0}'.format(self._feedback.time_passed))
        ...
        if success:
            self._result.transition = 't8:AlarmOff_L2'
            rospy.loginfo('ALARM L2 SERVER : TURNING OFF ALARM: {0}'.format(self._result.transition))
