...
class MinimalActionServer(object):
    ...
    def __init__(self, name):
        ...
        self._as.start()
        rospy.loginfo('SERVER 33 : ONLINE')
        self._as.start()
    ...
    def goal_callback(self, goal_request):
        ...
        self.get_logger().info('SERVER 33 : Received goal request')
        return GoalResponse.ACCEPT

    def cancel_callback(self, goal_handle):
        """Accepts or rejects a client request to cancel an action."""
        self.get_logger().info('SERVER 33 : Received cancel request')
        return CancelResponse.ACCEPT

    def execute_callback(self, goal):
        """Executes a goal."""
        rospy.loginfo('SERVER 33 : Executing goal...')
        ...
        for i in range(1, 5):
            if self._as.is_preempt_requested():
                ...
                success = False
                rospy.loginfo('SERVER 33 : Goal canceled')
                break
            # Update Fibonacci sequence
            self._feedback.time_passed.append(i)

            rospy.loginfo('SERVER 33 : Publishing feedback: {0}'.format(self._feedback.time_passed))
        ...
        if success:
            self._result.transition = 't33'
            rospy.loginfo('SERVER 33 : Returning result: {0}'.format(self._result.transition))
...
if __name__ == '__main__':
    rospy.init_node('action_server_33')
